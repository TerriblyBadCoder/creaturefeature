...
void main(){
    float dist = clamp(5*(0.1+length(texCoord-vec2(0.5,0.5))*0.9),1.0,6.0);

    vec2 fracted = texCoord;
    fracted.x -= fract(fracted.x*320.0)/320.0;
    fracted.y -= fract(fracted.y*180.0)/180.0;
    vec2 texEvil = fracted+vec2(
    sin(fracted.y*3.14*8.0+GameTime/20.0)/50.0*FadeInTest*dist/6.0f,
