...
uniform sampler2D SunDepthSampler;
in vec2 texCoord;
in vec2 oneTexel;
uniform vec2 InSize;
...
vec4 sobel(vec2 offsetex) {
    ...
    vec2 mosaicInSize =(vec2(1.0,1.0)/InSize)*3.0;
    vec2 offset[9] = vec2[](
    ...
    for (int i = 0; i < 9; i++) {
        ...
        if(texture(SunSampler, coord).a>0) {
    ...
    if(abs(Gy)>0.0||abs(Gx)>0.0){
        return vec4(66/255.0,4/255.0,6/255.0,1);
    }
    float G =0.0;
    return vec4(G, G, G, 0.0);
}
...
void main() {
    ...
    if(temp.r<0.9&&texture(SunDepthSampler,offsettex).r<texture(TrueDepthSampler,offsettex).r){
        ...
        if(sobeled.a>0.0){
            fragColor=sobeled;
            float noisy = min(abs(cnoise(vec3(offsettex*40.0,GameTime/6.0)))/1.14,1.0);
            noisy-=fract(noisy*7.0)/7.0;
            fragColor.rgb=fragColor.rgb*(1.0-noisy)+noisy*vec3(1.0,0.5,0.8);
        }
