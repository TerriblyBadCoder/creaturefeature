...
void main(){
    ...
    float noise = FadeInTest*((abs(cnoise(vec3(dirForNoise*0.7,GameTime/70.0f+20.0))*0.5)+0.2)*pow(length(fracted-vec2(0.5,0.5)),2.0)*2.0);
    float noise2 = FadeInTest*((abs(cnoise(vec3(dirForNoise,GameTime/50.0f+1000.0))*0.6)+0.2)*pow(length(fracted-vec2(0.5,0.5)),2.0)*2.0);
    if(noise>0.1||noise2>0.2){
        if(noise2<0.2)
            fragColor.rgb=fragColor.rgb*0.5+0.5*vec3(0.2,0.2,0.4);
        else{
            fragColor.rgb=vec3(0.2,0.2,0.4);
        }
    }
