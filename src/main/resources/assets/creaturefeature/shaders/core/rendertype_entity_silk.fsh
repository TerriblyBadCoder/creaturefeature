...
void main() {
    ...
    vec4 color = texture(Sampler4, fracCoord+vec2(-Time*1.4,sin(fracCoord.x*3.14/2.0+Time*2.0)/5.0))*0.67+texture(Sampler4, fracCoord*vec2(-1,1)+vec2(-Time*1.6,0.5+cos(fracCoord.x*3.14/2.0+Time*3.0)/5.0))*0.33;
    ...
    color.a=pow(color.a,1.0/length(color.rgb));
