    ...
    public void render(BlossomEntity entity, float entityYaw, float partialTicks, PoseStack poseStack, MultiBufferSource buffer, int packedLight) {
        ...
        for (int i = 0; i < 6; i++) {
            ...
            for (int j = 0; j < 6; j++) {
                ...
                vertex(pose,consumer,vec3.x,vec3.y,vec3.z,0,1.0f-(j+1)/6.0f,1,0.0f,1,packedLight,1.0f);
                vertex(pose,consumer,vec4.x,vec4.y,vec4.z,1,1.0f-(j+1)/6.0f,1,0.0f,1,packedLight,1.0f);
                vertex(pose,consumer,vec2.x,vec2.y,vec2.z,1,1.0f-(j)/6.0f,1,0.0f,1,packedLight,1.0f);
                vertex(pose,consumer,vec1.x,vec1.y,vec1.z,0,1.0f-(j)/6.0f,1,0.0f,1,packedLight,1.0f);
                vec1=vec3;
                vec2=vec4;
            }
            poseStack.popPose();
        ...
        for (int i = 0; i < 4; i++) {
            ...
            vertex(pose,consumer,vec3.x,vec3.y,vec3.z,i/4.0f,0.2f,1,0.0f,1,packedLight,0.5f);
            vertex(pose,consumer,vec4.x,vec4.y,vec4.z,(i+1)/4.0f,0.2f,1,0.0f,1,packedLight,0.5f);
            vertex(pose,consumer,vec2.x,vec2.y,vec2.z,(i+1)/4.0f,0,1,0.0f,1,packedLight,1.0f);
            vertex(pose,consumer,vec1.x,vec1.y,vec1.z,i/4.0f,0,1,0.0f,1,packedLight,1.0f);

            vertex(pose,consumer,vec3.x,vec3.y+0.8f,vec3.z,i/4.0f,1.0f,1,0.0f,1,packedLight,0.0f);
            vertex(pose,consumer,vec4.x,vec4.y+0.8f,vec4.z,(i+1)/4.0f,1.0f,1,0.0f,1,packedLight,0.0f);
            vertex(pose,consumer,vec4.x,vec4.y,vec4.z,(i+1)/4.0f,0.2f,1,0.0f,1,packedLight,0.5f);
            vertex(pose,consumer,vec3.x,vec3.y,vec3.z,i/4.0f,0.2f,1,0.0f,1,packedLight,0.5f);
            poseStack.popPose();
        }
