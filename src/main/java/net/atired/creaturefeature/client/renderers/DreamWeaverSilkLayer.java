...
@OnlyIn(Dist.CLIENT)
public class DreamWeaverSilkLayer<T extends DreamWeaverEntity, M extends DreamWeaverEntityModel<T>> extends EyesLayer<T, M> {
    ...
    private static final ResourceLocation SILK_OTHER =CreatureFeature.getId("textures/entity/dreamweaver_silk.png");

    private final DreamWeaverEntityModel<T> model;

    public DreamWeaverSilkLayer(RenderLayerParent<T, M> renderer, EntityModelSet modelSet) {
        super(renderer);
        this.model = new DreamWeaverEntityModel<>(modelSet.bakeLayer(DreamWeaverEntityModel.INNER_LAYER_LOCATION));
    }
