        ...
        if(myCube!=null&&!myCube.isRemoved()&&this.freshTickCount>2&&level() instanceof ServerLevel serverLevel){
            ...
            //myCube.getUniqueId()
             ...
             if(getTarget()!=null&&this.throwCD<18){
                 ...
                 vector3d=vector3d.mul(Math.min(14.0/vector3d.length(),1.0));
             }
             if(vector3d.y<0){
                 vector3d.y*=0.2;
             }
            ...
            pipeline.addLinearAndAngularVelocity(myCube,vector3d,new Vector3d(dir.x*0.1,0.3,dir.z*0.1));
        }
        if(myClientCube==null&&level() instanceof ClientLevel clientLevel){
            BlockPos pos = getOnPos().offset(0,1,0);

        }
        if(myCube==null&&this.freshTickCount>0&&position().y>-60&&level() instanceof ServerLevel serverLevel){
