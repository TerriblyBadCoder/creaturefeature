...
public class DownFeatherItem extends Item implements ProjectileItem {
    public DownFeatherItem(Item.Properties properties) {
        super(properties);
    }
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand hand) {
        ...
        if (!level.isClientSide) {
            for (int i = 0; i < 3; i++) {
                FeatherEntity snowball = new FeatherEntity(level, player);
...
//    }
    public Projectile asProjectile(Level level, Position pos, ItemStack stack, Direction direction) {
        FeatherEntity snowball = new FeatherEntity(level, pos.x(), pos.y(), pos.z());
